...
class Device extends OAuth2Device {
  ...
  */
  ...
  async triggerFlows(data) {
    ...
    // In geofence trigger
    if (data.in_geofence && this.hasCapability('in_geofence') && !this.getCapabilityValue('in_geofence')) {
      this.driver.inGeofenceTrueTrigger
        .trigger(device, { geofence: data.geofence })
        .then()
        .catch(device.error);
    }

    // In Power Saving Zone trigger
    if (data.in_power_saving_zone && this.hasCapability('in_power_saving_zone') && !this.getCapabilityValue('in_power_saving_zone')) {
      this.driver.inPowerSavingZoneTrueTrigger
        .trigger(device, { power_saving_zone: data.power_saving_zone })
        .then()
        .catch(device.error);
    }
