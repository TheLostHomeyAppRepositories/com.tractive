  ...
  */
  ...
  async discoverTrackers() {
    ...
    for (const entry of trackers) {
      if (blank(entry) || blank(entry._id || null)) continue;
  ...
  async discoverTracker(id) {
    ...
    if (blank(device) || blank(device._id || null)) return null;
  ...
  */
  ...
  async getPets() {
    ...
    for (const entry of result) {
      if (blank(entry._id || null)) continue;

      // Retrieve pet
      const pet = await this.getPet(entry._id);

      // Ignore empty pets
      if (blank(pet) || blank(pet._id || null)) continue;
  ...
  */
  ...
  async getSubscriptions() {
    ...
    for (const entry of result) {
      ...
      if (blank(sub) || blank(sub._id || null)) continue;
